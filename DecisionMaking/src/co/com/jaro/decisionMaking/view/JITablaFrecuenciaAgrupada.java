 ...
 *
 */
public class JITablaFrecuenciaAgrupada extends JInternalFrame {

	private static final long serialVersionUID = -918458381820634877L;
	...
	private ArrayList<NodoAgrupado> nodoAgrupados = new ArrayList<NodoAgrupado>();
	private ArrayList<String> tablaFrecAgrupada = new ArrayList<String>();
	
	public JITablaFrecuenciaAgrupada() {
		setFrameIcon(new ImageIcon(JITablaFrecuenciaAgrupada.class.getResource("/co/com/jaro/decisionMaking/resources/img/table_group.png")));
		setTitle("Tabla de frecuencia agrupada");
			...
			public void actionPerformed(ActionEvent e) {
	            ...
	            if (opcion == JFileChooser.APPROVE_OPTION) {
	                ...
	                if(!txtCargarArchivo.getText().substring((txtCargarArchivo.getText().length() - 3), txtCargarArchivo.getText().length())
	                	...
	                	try {
	                		tablaFrecAgrupada = ManejoArchivoController.leerArchivoExcel(txtCargarArchivo.getText());
							

						  	nodoAgrupados = (ArrayList<NodoAgrupado>) TableController.mostrarDatos(
						  			tabla, 
						  			modelo, 
						  			Constantes.columnasAgrupada(tablaFrecAgrupada.get(0)),
						  			tablaFrecAgrupada, 
						  			"agrupada");
		...
		btnGenerarArchivo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (txtCargarArchivo.getText().toString().trim().equals("")) {
					...
					ManejoArchivoController.archivoSalidaAgrupada(nodoAgrupados, rutaDestino, "Tabla agrupada", tablaFrecAgrupada.get(0));
					 
					lblMsg.setText("Archivo generado exitosamente!!");
					pnlMsg.setBorder(new LineBorder(Color.GREEN));
					
					GraficoTorta.generarTortaAgrupado(nodoAgrupados);
					GraficoHistograma.generarHistogramaAgrupado(nodoAgrupados);
				}
			}
		});
	}
